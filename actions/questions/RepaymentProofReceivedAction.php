...
$checkIfLoanExists = $bdd->prepare('SELECT * FROM loan WHERE id = ? AND id_lender= ? AND repayment_received="no_correct_id"');
...
if($checkIfLoanExists->rowCount() > 0){
	...
	$phone_number_display = "Unknown";
			...
			if($repayment == 'received'){
				...
				$confirmed_message ="<div class='popup-confirmed'><div class='confirmed-div'><div class='subtitle-confirmed'><span>Repayment Confirmed</span></div><img class='confirmed-image' src='assets/images/success.jpg'><a href='dashboard.php'><button class='confirmed-button'>Go Home</button></a></div></div>";
			...
			if($repayment == 'not_received'){
				...
				$SetPaymentNotmade->execute(array($idOfLoan));
						...
						$notreceived_message = "<div class='popup-notconfirmed'><div class='notconfirmed-div'><div class='subtitle-notconfirmed'><span>Repayment Not Received</span></div><div class='notconfirmed-text'><span>Repayment Reported as Not Received. Borrower will Be Banned for Fraud.</br>You will be Updated by Email.</span></div><a href='dashboard.php'><button class='notconfirmed-button'>Go Home</button></a></div></div>";
				}
